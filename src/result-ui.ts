...
export function showResults(rows: ResultRow[], winnerId: number): void {
    ...
    const panel = new UIContainer({
        ...
        y: 40,
        width: 560,
        height: 60 + Math.min(rows.length, MAX_ROWS) * 30,
    ...
    // Column header row.
    const header = (msg: string, x: number, w: number, anchor: mod.UIAnchor) =>
        new UIText({
            parent: panel,
    ...
    header('#', 16, 30, mod.UIAnchor.CenterLeft);
    header('PLAYER', 52, 260, mod.UIAnchor.CenterLeft);
    header('GUN', 330, 70, mod.UIAnchor.Center);
    header('K', 430, 50, mod.UIAnchor.Center);
    header('D', 494, 50, mod.UIAnchor.Center);
    ...
    for (let i = 0; i < shown.length; i++) {
        ...
        const cell = (msg: mod.Any, x: number, w: number, anchor: mod.UIAnchor, size: number) =>
            new UIText({
                parent: panel,
                anchor: mod.UIAnchor.TopLeft,
                x,
        ...
        cell(i + 1, 16, 30, mod.UIAnchor.CenterLeft, 16);
        cell(r.player, 52, 260, mod.UIAnchor.CenterLeft, 16); // Message(player) => name
        cell(`${r.gun}/${r.total}`, 330, 70, mod.UIAnchor.Center, 16);
        cell(r.kills, 430, 50, mod.UIAnchor.Center, 16);
        cell(r.deaths, 494, 50, mod.UIAnchor.Center, 16);
    }

    // Per-player VICTORY / DEFEAT title + VO.
    for (const r of rows) {
        const isWinner = mod.GetObjId(r.player) === winnerId;
        ...
        new UIText({
            ...
            width: 700,
            height: 90,
            message: mod.Message(isWinner ? 'VICTORY' : 'DEFEAT'),
            textSize: 64,
