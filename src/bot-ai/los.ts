...
import { allAliveEnemies, Tlm } from './ffa-deps.ts';
import { isTargetProtected } from './spawn-protect.ts';
...
const EYE_FORWARD = 0.5; // push the ray start forward so it clears the caster's own ~0.3-0.4m capsule.
// Kept as SMALL as safely possible: a larger offset means a wall-hugging bot's ray STARTS on the far
// side of the wall (never sees it), reports a false "clear", and the bot shoots through solid geometry.
// 0.5 clears the caster's body while shrinking that punch-through band. Tune 0.4-0.6 in-game.
...
export function updateLos(bots: mod.Player[], sightRange: number): void {
    ...
    for (let i = 0; i < n; i++) {
        ...
        try {
            ...
            const enemies = allAliveEnemies(bot); // FFA: everyone else is an enemy
            ...
            for (const en of enemies) {
                try {
                    if (isTargetProtected(en)) continue; // fresh spawn — don't spend the LOS ray on them
